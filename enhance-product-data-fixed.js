...
async function enhanceProductData() {
  ...
  // 3. Заполняем недостающую информацию
...
async function fillMissingInformation(products) {
  ...
  for (const product of products) {
    ...
    // Заполняем недостающие поля на основе типа продукта
...
async function saveEnhancedProducts(products) {
  ...
  await fs.mkdir('./src/data/pdfs', { recursive: true });
  
  // Сохраняем обновленные данные в соответствующие файлы
  for (const [type, typeProducts] of Object.entries(groupedByType)) {
